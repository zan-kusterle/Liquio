      ...
      if depth >= 0 do
        ...
        results_by_units =
          ...
          |> Enum.map(fn {unit_value, votes_for_unit} ->
            votes_for_unit =
              if is_unit_spectrum(unit_value) do
      ...
      if depth >= 1 do
        reference_votes =
          ...
          |> Enum.filter(fn vote ->
            ...
            key === node.key or
              String.starts_with?(key, "#{String.trim_trailing(node.key, "/")}/")
          end)
        ...
        references =
          ...
          |> Enum.flat_map(fn {_key, votes} ->
            ...
            |> Enum.map(fn {_title, title_votes} ->
              ...
              |> Node.new()
              |> Map.put(:referencing_title, Average.mode(Enum.map(title_votes, & &1.title)))
              |> Map.put(
                :reference_results,
                Liquio.Results.from_votes(title_votes, inverse_delegations)
              )
              |> Node.load(data, depth - 1)
            end)
          end)

        inverse_reference_votes =
          all_reference_votes
          |> Enum.filter(fn vote ->
