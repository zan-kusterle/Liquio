defmodule Liquio.Results do
  ...
  def from_votes(votes, inverse_delegations, %{:datetime => datetime, :topics => topics}) do
    ...
    inverse_delegations =
      inverse_delegations
      |> Enum.filter(fn {from_identity_username, _} ->
  ...
  defp from_contributions(contributions, datetime) do
    ...
    latest_contributions =
      contributions_by_identities |> Enum.map(fn {_, cs} -> cs |> List.last() end)
      ...
      if vote_weight_halving_days == nil do
        ...
        |> Enum.map(fn contribution ->
          Map.put(
            contribution,
            :voting_power,
            contribution.voting_power *
              moving_average_weight(contribution, datetime, vote_weight_halving_days)
          )
        end)
      end
