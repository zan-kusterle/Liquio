...
var _updateRule = __webpack_require__(52);
...
var _SheetsRegistry = __webpack_require__(10);

var _SheetsRegistry2 = _interopRequireDefault(_SheetsRegistry);

var _RulesContainer = __webpack_require__(2);
...
var _getDynamicStyles = __webpack_require__(51);
...
exports.slug = slug;
exports.colorOnGradient = colorOnGradient;
...
function colorOnGradient(color_a, color_b, ratio) {
...
function defaultUnit(node) {
    ...
    if (node.results) {
        ...
        for (var unitKey in node.results.by_units) {
            if (maxUnitKey == null || node.results.by_units[unitKey].turnout_ratio > node.results.by_units[maxUnitKey].turnout_ratio) maxUnitKey = unitKey;
        }
...
var _jssExtend = __webpack_require__(31);

var _jssExtend2 = _interopRequireDefault(_jssExtend);

var _jssNested = __webpack_require__(33);

var _jssNested2 = _interopRequireDefault(_jssNested);

var _jssCamelCase = __webpack_require__(25);
...
var _jssVendorPrefixer = __webpack_require__(35);
...
var _jssCompose = __webpack_require__(26);

var _jssCompose2 = _interopRequireDefault(_jssCompose);

var _jssExpand = __webpack_require__(29);

var _jssExpand2 = _interopRequireDefault(_jssExpand);

var _jssGlobal = __webpack_require__(32);
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(14), __webpack_require__(53)(module)))
...
var _RegularRule = __webpack_require__(44);

var _RegularRule2 = _interopRequireDefault(_RegularRule);

var _cloneStyle = __webpack_require__(48);
...
var _VirtualRenderer = __webpack_require__(40);
...
var _jssPresetDefault = __webpack_require__(6);
...
var _text2 = __webpack_require__(56);

var text = _interopRequireWildcard(_text2);
...
var _crossStorage = __webpack_require__(20);
...
if (key.startsWith("http://localhost:4000" + '/page/')) key = decodeURIComponent(key.replace("http://localhost:4000" + '/page/', ''));
key = utils.cleanUrl(key);

if (!window.location.href.startsWith("http://localhost:4000" + '/n/') || window.location.href.startsWith("http://localhost:4000" + '/page')) {
    var storage = new _crossStorage.CrossStorageClient("http://localhost:4000" + '/hub.html');
    var storageUsernames = void 0,
        storageTrustMetricURL = void 0;
    storage.onConnect().then(function () {
        storage.get('usernames').then(function (usernames) {
            storageUsernames = usernames.split(',');
            if (storageTrustMetricURL) onDataReady(storageUsernames, storageTrustMetricURL);
        });
        storage.get('').then(function (value) {
            storageTrustMetricURL = value;
            if (storageUsernames) onDataReady(storageUsernames, storageTrustMetricURL);
        });
    }).catch(function () {
        onDataReady([], null);
    });
...
var onDataReady = function onDataReady(usernames, trustMetricURL) {
    ...
    utils.getUrl("http://localhost:4000" + '/api/nodes/' + encodeURIComponent(key) + '?depth=2&trust_usernames=' + usernames.join(','), function (data) {
        onResultsReady(data.data, trustMetricURL);
    });
};

var onResultsReady = function onResultsReady(node, trustMetricURL) {
    bar.init(key, trustMetricURL, node.results.by_units['reliable'], classes);

    var nodes_by_text = get_nodes_by_text(node);

    text.init(nodes_by_text, classes);
    video.setData(nodes_by_text);

    if (!key.startsWith("http://localhost:4000" + '/n/')) {
        initDomListener();
    }
};

var onDomNodeInsert = function onDomNodeInsert(node) {
...
function initDomListener() {
    ...
    if (MutationObserver) {
        var obs = new MutationObserver(function (mutations, observer) {
            _lodash2.default.each(mutations, function (mutation) {
                _lodash2.default.each(mutation.addedNodes, function (node) {
        ...
        document.addEventListener('DOMNodeInserted', function (e) {
            if (e.target.nodeName.toLowerCase() == 'a') onAnchorInsert(e.target);
        }, false);
    }
}

var get_nodes_by_text = function get_nodes_by_text(node) {
    var nodes_by_text = {};
    node.references.forEach(function (reference) {
        reference.inverse_references.forEach(function (inverse_reference) {
            var topic = inverse_reference.path.join('/');
            if (topic.startsWith(key + '/')) {
                var remainder = topic.substring(key.length + 1);
                if (remainder.length > 0) {
                    var _text = remainder.replace(/-/g, ' ');
                    if (!(_text in nodes_by_text)) nodes_by_text[_text] = [];
                    nodes_by_text[_text].push(reference);
                }
            }
        });
    });
...
module.exports = {
  CrossStorageClient: __webpack_require__(18),
  CrossStorageHub: __webpack_require__(19)
};
...
var _prefix = __webpack_require__(5);

var _prefix2 = _interopRequireDefault(_prefix);

var _supportedProperty = __webpack_require__(23);

var _supportedProperty2 = _interopRequireDefault(_supportedProperty);

var _supportedValue = __webpack_require__(24);
...
var _defaultUnits = __webpack_require__(27);
...
var _cssVendor = __webpack_require__(22);
...
var _generateClassName = __webpack_require__(50);
...
var _KeyframeRule = __webpack_require__(43);
...
var _ViewportRule = __webpack_require__(46);
...
var defaultTrustMetricURL =  false ? 'https://trust-metric.liqu.io' : 'http://127.0.0.1:8080/dev_trust_metric.html';

function init(key, trustMetricURL, reliability_results, classes) {
    var rating = reliability_results ? reliability_results.average : null;

    var red = 'B51212',
        yellow = 'FCF119',
        green = '44DD25';
    var color = '33bae7';
    if (rating) color = rating < 0.5 ? utils.colorOnGradient(yellow, red, rating * 2) : utils.colorOnGradient(green, yellow, (rating - 0.5) * 2);

    var trustMetricInputElement = document.createElement('input');
    ...
    var togglableElement = document.createElement('div');
    ...
    scoreElement.addEventListener('click', function (e) {
        var isShown = togglableElement.style.display !== 'none';
    ...
    viewElement.addEventListener('click', function (e) {
        ...
        var url = "http://localhost:4000" + '/n/' + encodeURIComponent(key + anchor) + '/Reliable-Unreliable';
...
exports.addToContainer = addToContainer;
function prepareContainer(container, classes) {
    var buttonElement = document.createElement('a');
    buttonElement.className = 'liquio-button ' + classes.button;
    buttonElement.innerHTML = '<img src="' + "http://localhost:4000" + '/images/icon.svg" width="24" height="24"></img>';
    ...
    buttonElement.addEventListener('click', function (e) {
        var isShown = noteElement.style.display === 'block';

        var nodes = document.getElementsByClassName('liquio-note');
...
function addToContainer(container, node) {
    var note = container.parentNode.getElementsByClassName('liquio-note')[0];

    var units = node.results ? Object.values(node.results.by_units) : [];
    var bestUnit = _.maxBy(units, function (u) {
        return u.turnout_ratio;
    });

    var anchorElement = document.createElement('a');
    anchorElement.setAttribute('href', "http://localhost:4000" + '/' + encodeURIComponent(node.path.join('/')));
...
exports.init = init;
exports.onAnchorInsert = onAnchorInsert;
exports.onNodeInsert = onNodeInsert;
...
var note = _interopRequireWildcard(_note);
...
function init(_nodesByText, _classes) {
...
function onAnchorInsert(node) {
    var is_http = node.protocol === 'http:' || node.protocol === 'https:';
    var is_blank = node.target == "_blank";

    var a_hostname = node.hostname.split('.');
    var a_domain = a_hostname[a_hostname.length - 2];

    var current_hostname = window.location.hostname.split('.');
    var current_domain = current_hostname.length >= 2 ? current_hostname[current_hostname.length - 2] : '';

    if (is_http && (a_domain !== current_domain || is_blank)) {
        var url = utils.cleanUrl(node.origin + node.pathname);

        var startsWith = "http://localhost:4000".replace('://', ':') + '/';
        if (url.startsWith(startsWith)) {
            url = url.substring(startsWith.length);
        }

        utils.getUrl("http://localhost:4000" + '/api/nodes/' + encodeURIComponent(url), function (data) {
            var view = tag.getAttribute('liquio-view') || 'default';
            var defaultUnit = utils.defaultUnit(data.data);
            ...
            if (defaultUnit) {
                if (view == 'default') {
                    ...
                    var yellow = 'FCF119';
                    var green = '44DD25';
                    var color = defaultUnit.average < 0.5 ? utils.colorOnGradient(yellow, red, defaultUnit.average * 2) : utils.colorOnGradient(green, yellow, (defaultUnit.average - 0.5) * 2);
...
function onNodeInsert(domNode) {
    var nodesWithText = Object.keys(nodesByText).filter(function (k) {
        return domNode.textContent.toLowerCase().indexOf(k) >= 0;
    }).reduce(function (a, k) {
        return a.concat(nodesByText[k]);
    }, []);
    ...
    if (nodesWithText.length > 0) {
        ...
        try {
            for (var _iterator2 = nodesWithText[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                var nodeWithText = _step2.value;
...
function setup() {
    ...
    var overlay = player.getElementsByClassName('liquio-video-note')[0];

    var isOverlayOpen = false;
    var currentNode = null;

    var update = function update() {
        var time = instance.getCurrentTime();

        var newNode = null;
        for (var key in nodesByText) {
            var key_parts = key.replace('/', '').split(":");
            if (key_parts.length == 2) {
                var key_time = parseInt(key_parts[0]) * 60 + parseInt(key_parts[1]);
                var delta = time - key_time;
                if (delta >= 0 && delta <= 10) {
        ...
        if (newNode == null) {
            if (!isOverlayOpen) overlay.style.opacity = 0;
        } else {
            if (currentNode == null || !_lodash2.default.isEqual(newNode.path, currentNode.path)) {
...
function onYouTubeIframeAPIReady() {
    var player = new YT.Player('main-video-frame', {
        events: {
