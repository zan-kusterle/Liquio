defmodule Liquio.HtmlReferenceController do
	...
	def show(conn, params = %{:user => user, :nodes => nodes, :reference_nodes => reference_nodes}) do
		...
		if Enum.count(node_unique_choice_types) > 1 or Enum.count(reference_node_unique_choice_types) > 1 do
			...
			for_choice_node = if first_node.choice_type == nil do nil else first_node |> Map.put(:title, "Choice For Which Reference Poll Provides Evidence") end
			relevance_node =  first_node |> Map.put(:title, "How Strongly?") |> Map.put(:choice_type, "probability")
